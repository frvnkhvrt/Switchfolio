...
const EnhancedProjectBox: React.FC<EnhancedProjectBoxProps> = ({
  status,
  title,
  content,
  url,
              ...
              <div className="flex flex-wrap gap-1.5 select-none" role="list" aria-label="Technologies used">
                {skill.map((skillName, index) => (
                  <motion.span
                    key={index}
                    role="listitem"
                    className="border-2 border-primaryBlue px-2 py-0.5 rounded-none text-sm font-medium dark:border-folderCream dark:text-folderCream"
...
export default EnhancedProjectBox